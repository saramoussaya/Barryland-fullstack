...
const statuses = ['nouveau', 'lu', 'traite'];

const StatusBadge: React.FC<{ status: MessageItem['status'] }> = ({ status }) => {
...
const AdminMessages: React.FC = () => {
  ...
  useEffect(() => { load(); }, [page, filterStatus, filterProperty]);

  const openDetail = async (m: MessageItem) => {
    try {
  ...
  const changeStatus = async (id: string, status: MessageItem['status']) => {
    try {
  ...
  return (
    <div className="p-4 md:p-6">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
        <h2 className="text-2xl font-semibold tracking-tight">Messages reçus</h2>
        <div className="flex items-center gap-3">
          ...
          <input placeholder="Filtrer par propertyId" value={filterProperty} onChange={e => { setFilterProperty(e.target.value); setPage(1); }} className="px-3 py-2 border rounded text-sm" />
          <button onClick={load} className="px-3 py-2 bg-emerald-600 text-white rounded text-sm">Actualiser</button>
          <Link to="/admin" className="text-sm text-gray-600">Retour</Link>
        </div>
      </div>
